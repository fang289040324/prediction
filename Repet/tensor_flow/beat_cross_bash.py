...
def main(fold):
    ...
    output_file = output_folder + 'output_{0}_folds_{1}_epochs.txt'.format(n_folds, n_epochs)
    ...
    print('AVG = ', np.mean(train_sdr))
    pprint.pprint(prediction)
    predicted.append(prediction)
    with open(output_file, 'a') as f:
        mse = np.square(test_sdr - prediction).mean()
        f.write('Fold {0}\t mse = {1} dB \t time = {2} sec \n'.format(fold + 1, mse, elapsed))
