    ...
    for f in os.listdir(src_dir):
        for h in os.listdir(ir_dir):
            generate_reverb(src_dir + f, ir_dir + h,
                            dest_dir+os.path.splitext(f)[0] + '_' + os.path.splitext(h)[0] +'.wav')


def generate_duet_histograms(src_dir, out_dir):
    for f in os.listdir(src_dir):
        ...
        duet = nussl.Duet(sig, 2, sample_rate=sr)

        if not os.path.exists(out_dir):
            os.mkdir(out_dir)
        duet.run()

        duet.plot(out_dir + os.path.splitext(f)[0] + '_2d.png')
        duet.plot(out_dir + os.path.splitext(f)[0] + '_3d.png', True)

        output_name_stem = out_dir + os.path.splitext(f)[0] + '_duet_source'
        i = 1
...
def generate_reverb(signal, reverb, fname):
    ...
    wet_mix = add_reverb(data.T, data_ir.T)

    scipy.io.wavfile.write(fname, sr, wet_mix.T)
